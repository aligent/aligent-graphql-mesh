...
import { BC_CountryCode, BC_DistanceFilter, BC_InventoryLocationConnection, BC_InventoryLocationsArgs } from "@mesh/external/BigCommerceGraphqlApi";

export const getTransformedStoreLocationsArgs = (acStoreLocationsArgs: QuerystoreLocationsArgs): BC_InventoryLocationsArgs =>{
    const { area, filters } = acStoreLocationsArgs;
    const applyDistanceFilter = area?.coordinates && Object.keys(area?.coordinates).length && area?.radius && area?.search_term;
    ...
    if(area){
        ...
        if(coordinates && Object.keys(coordinates).length && radius) {
            ...
            (bc_InventoryLocationsArgs.distanceFilter as BC_DistanceFilter).lengthUnit = 'Kilometres';

            const { lat, lng } = coordinates;
            (bc_InventoryLocationsArgs.distanceFilter as BC_DistanceFilter).longitude = lng as number;
            (bc_InventoryLocationsArgs.distanceFilter as BC_DistanceFilter).latitude = lat as number;
        }
    }
...
export const getTransformedStoreLocationItems = (bcStoreLocations: BC_InventoryLocationConnection): StoreLocations => {
