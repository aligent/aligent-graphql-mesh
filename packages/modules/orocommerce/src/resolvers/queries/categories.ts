...
import { CategoriesTransformerChain,  decodeCategoryId } from '../../transformers/categories/categories-transformer';

export const categoriesResolver: QueryResolvers['categories'] = {
    resolve: async (_root, args, context, _info) => {
        const categoryUid = <string>args?.filters?.category_uid?.eq;
