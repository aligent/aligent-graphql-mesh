...
describe('Cms Blocks data transformation tests', () => {
    ...
    test('Check whether the transformed cms blocks contain expected data', () => {
        ...
        for (const blockIndex in output.items!) {
            ...
            expect(transformedBlock.content).toStrictEqual(`${blockAttrs.contentVariant.content}<style>${blockAttrs.contentVariant.style}</style>`);
            expect(transformedBlock.identifier).toStrictEqual(mockBlocks[blockIndex].attributes.alias);
